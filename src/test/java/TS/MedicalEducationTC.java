...
public class MedicalEducationTC extends BaseTest {
    @Test

    public void verifyMedicalEducationSLT() throws InterruptedException {
        ...
        MedicalEducationPO medEduPOObj = new MedicalEducationPO(driver);

        //*****************Credentialing Info - Medical Education Tab*********************//
        ExtentTestManager.getTest().getTest().setName("SMT-004: Verify Medical Education Page ");
        //1. Click dropdown arrow on profile user
        Assert.assertTrue(medEduPOObj.clickOnLinkBtn(medEduPOObj.DropDownPU_xpath), "Unable to click on profile user dropdown");
        ...
        Assert.assertTrue(medEduPOObj.clickOnLinkBtn(medEduPOObj.CreInfo_xpath), "Unable to click on Credential Info link");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Click on Credential Info link ", "User able to click on Credential Info link");

        //3. Click on Medical Education tab
        Assert.assertTrue(medEduPOObj.clickOnLinkBtn(medEduPOObj.MedEduTab_xpath), "Unable to click on Medical Education tab");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Click on Medical Education tab ", "User able to click on Medical Education tab");

        //3. Click on Add new btn
        Assert.assertTrue(medEduPOObj.clickOnLinkBtn(medEduPOObj.MedEduAddNew_xpath), "Unable to click on add new btn");
        ...
        String expectedMessage = "A new medical education entry has been successfully created.";
